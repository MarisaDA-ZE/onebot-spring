...
import top.kirisamemarisa.onebotspring.core.util.CQMessageUtil;
...
//        // 只监控群聊
//        if (messageType != MessageType.GROUP) return false;
//
//        // 触发
...
//        }
    ...
    private void friendHandler(GroupReport groupReport) {
        ...
        String msg = "私聊不能揉胸(´•ω•̥`)";
        MessageType type = MessageType.PRIVATE;
        String template = CQMessageTemplate.textListTemplate(type, sender.getUserId(), List.of(msg));
    ...
    private void groupHandler(GroupReport groupReport) {
        ...
        String url = config.getClientUrl() + ClientApi.SEND_MSG.getApiURL();
        ...
        if (StrUtil.isNotEmpty(params.getError())) {
            String err = params.getError();
            CQMessageTemplate.textListTemplate(MessageType.GROUP, groupId, List.of(err));
            return;
        }

        if (params.getCount() > 50) {
            String content = "揉太多次啦（每回最多揉50次）(´•ω•̥`)";
            CQMessageUtil.sendTextMessage(url, MessageType.GROUP, groupId, content);
            return;
        }
        ...
        if (ObjectUtils.isEmpty(sexUser)) {
            ...
            String content = "您（在本群）还未注册账号，请先注册后使用，注册过程请查看帮助" + Emoji.SORROW_9.getEmoji();
            CQMessageUtil.sendTextMessage(url, MessageType.GROUP, groupId, content);
            return;
        }
        ...
        template = CQMessageTemplate.textListTemplate(MessageType.GROUP, groupId, List.of(text));
        HttpUtils.post(url, template);
    }
